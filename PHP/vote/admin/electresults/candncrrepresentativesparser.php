...
<TITLE>Vote.ph : Election Results Input on NCR Representatives Candidates</TITLE>
</HEAD>
...
<IMG SRC="/vote/graphics/rightarrow.gif" WIDTH="25" HEIGHT="12" BORDER="0" ALT="-->">
<B>NCR Representatives Candidates (Parser)</B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>ELECTION RESULTS INPUT ON NCR REPRESENTATIVES CANDIDATES (VIA PARSER)</B></DIV>
<BR>
<BR>
...
<B>Synax:</B><BR>
[Last Name],[First Name],[NCR City or Municipality],[Leg. District No.],[No. of Votes],[Is Proclaimed?];<BR>
<B>Examples of inputs:</B><BR>
Arroyo,Joker,Makati,1,500,Y;<BR>
<FORM ACTION=<?PHP echo $PHP_SELF; ?> METHOD="post">
<B>Enter representatives input in comma-delimited format:</B><BR>
...
for ($ctr=0;$ctr < sizeof($linearray)-1;$ctr++) {
	...
	list($lastname,$firstname,$ncrmunicity,$distnum,$numvotes,$isproclaimed) = split(",",$linearray[$ctr]);
	$query	= "SELECT legdistricts.legdist_id FROM legdistricts, nationalcapitalregion WHERE (nationalcapitalregion.municity_id = legdistricts.ncrmunicity_id) AND 
	                   (UCASE(nationalcapitalregion.name) = '".strtoupper(trim($ncrmunicity))."') AND (legdistricts.dist_num = ".strval($distnum).")";
	echo $query."<BR>";		
	$legdistricts = getqueryresults($query);
	$legdistrictsrow = mysql_fetch_array($legdistricts);
	$query	= "UPDATE candrepresentatives SET numvotes=".strval($numvotes).", is_proclaimed='".trim(strtoupper($isproclaimed)).
	          "' WHERE (UCASE(lastname) = '".strtoupper(trim($lastname))."') AND (UCASE(firstname) = '".strtoupper(trim($firstname)).
			  "') AND (legdist_id = ".strval($legdistrictsrow['legdist_id']).")";
	echo $query."<BR>";		  
