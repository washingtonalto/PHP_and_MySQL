...
<TITLE>Vote.ph : Election Results Input on NCR Vice Mayoral Candidates</TITLE>
</HEAD>
...
<IMG SRC="/vote/graphics/rightarrow.gif" WIDTH="25" HEIGHT="12" BORDER="0" ALT="-->">
<B>NCR Vice Mayoral Candidates (Parser)</B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>ELECTION RESULTS INPUT ON NCR VICE MAYORAL CANDIDATES (VIA PARSER)</B></DIV>
<BR>
<BR>
...
<B>Synax:</B><BR>
[Last Name],[First Name],[NCR City or Municipality],[No. of Votes],[Is Proclaimed?];<BR>
<B>Examples of inputs:</B><BR>
Arroyo,Joker,Makati,500,Y;<BR>
<FORM ACTION=<?PHP echo $PHP_SELF; ?> METHOD="post">
<B>Enter ncr vice mayor input in comma-delimited format:</B><BR>
...
for ($ctr=0;$ctr < sizeof($linearray)-1;$ctr++) {
	...
	list($lastname,$firstname,$ncrmunicity,$numvotes,$isproclaimed) = split(",",$linearray[$ctr]);
	$query	= "SELECT nationalcapitalregion.municity_id FROM nationalcapitalregion WHERE (nationalcapitalregion.name = '".trim(strtoupper($ncrmunicity))."')";
	echo $query."<BR>";		
	$ncrmunicity = getqueryresults($query);
	$ncrmunicityrow = mysql_fetch_array($ncrmunicity);
	$query	= "UPDATE candvicemayors SET numvotes=".strval($numvotes).", is_proclaimed='".trim(strtoupper($isproclaimed)).
	          "' WHERE (UCASE(lastname) = '".strtoupper(trim($lastname))."') AND (UCASE(firstname) = '".strtoupper(trim($firstname)).
			  "') AND (ncrmunicity_id = ".$ncrmunicityrow['municity_id'].")";
	echo $query."<BR>";		  
