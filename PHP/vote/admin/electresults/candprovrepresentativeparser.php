...
<TITLE>Vote.ph : Election Results Input on Provincial Representatives Candidates</TITLE>
</HEAD>
...
<IMG SRC="/vote/graphics/rightarrow.gif" WIDTH="25" HEIGHT="12" BORDER="0" ALT="-->">
<B>Provincial Representatives Candidates (Parser)</B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>ELECTION RESULTS INPUT ON PROVINCIAL REPRESENTATIVES CANDIDATES (VIA PARSER)</B></DIV>
<BR>
<BR>
...
<B>Synax:</B><BR>
[Last Name],[First Name],[Province],[Leg. District No.],[No. of Votes],[Is Proclaimed?];<BR>
<B>Examples of inputs:</B><BR>
Arroyo,Joker,Pangasinan,1,500,Y;<BR>
...
for ($ctr=0;$ctr < sizeof($linearray)-1;$ctr++) {
	...
	list($lastname,$firstname,$province,$distnum,$numvotes,$isproclaimed) = split(",",$linearray[$ctr]);
	$query	= "SELECT legdistricts.legdist_id FROM legdistricts, provinces WHERE (provinces.province_id = legdistricts.province_id) AND 
	                   (UCASE(provinces.name) = '".strtoupper(trim($province))."') AND (legdistricts.dist_num = ".strval($distnum).")";
	echo $query."<BR>";		
	...
	$query	= "UPDATE candrepresentatives SET numvotes=".strval($numvotes).", is_proclaimed='".trim(strtoupper($isproclaimed)).
			  ...
			  "') AND (legdist_id = ".$legdistrictsrow['legdist_id'].")";
	echo $query."<BR>";		  
