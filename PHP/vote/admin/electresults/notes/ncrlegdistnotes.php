...
<TITLE>Vote.ph : NCR Legislative District Election Results Notes</TITLE>
</HEAD>
...
<IMG SRC="/vote/graphics/rightarrow.gif" WIDTH="25" HEIGHT="12" BORDER="0" ALT="-->">
<B>NCR Legislative District Election Results Notes</B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>NCR LEGISLATIVE DISTRICT ELECTION RESULTS NOTES</B></DIV>
<BR>
...
<B>Synax:</B><BR>
[NCR City or Municipality],[District Number],[Display House Representative Results (Y/N)?],[House Representative Notes];<BR>
<B>Examples of inputs:</B><BR>
Malabon,1,Y,As of 5:30 pm Comelec;<BR>
<FORM ACTION=<?PHP echo $PHP_SELF; ?> METHOD="post">
<B>Enter NCR legislative district input in comma-delimited format:</B><BR>
...
for ($ctr=0;$ctr < sizeof($linearray)-1;$ctr++) {
	...
	list($ncrmunicity,$districtnum,$isdisprepresult,$repcomment) = split(",",$linearray[$ctr]);
	$query = "SELECT legdistricts.legdist_id FROM legdistricts, nationalcapitalregion WHERE legdistricts.dist_num = ".strval($districtnum).
	         " AND UCASE(nationalcapitalregion.name) = '".strtoupper(trim($ncrmunicity))."' AND legdistricts.ncrmunicity_id = nationalcapitalregion.municity_id";
	echo $query."<BR>";
	$legdistricts = getqueryresults($query);
	$legdistrictsrow = mysql_fetch_array($legdistricts);		  
	$query	= "UPDATE legdistricts SET is_disprepresults='".trim($isdisprepresult).
	          "', represultcomment='".trim($repcomment).
			  "' WHERE legdist_id = ".$legdistrictsrow['legdist_id']; 
	echo $query."<BR>";		  
