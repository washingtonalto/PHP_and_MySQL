...
if (empty($hassubmit)) {
    $query = "SELECT  activity_id, body, date, party_id, province_id, municity_id, ncrmunicity_id, is_national,
             source, title
       FROM activities
       WHERE activity_id = ".$activityid;
    $activity =  getqueryresults($query);
    $activityrow = mysql_fetch_array($activity);
...
<!--======================= End of MetaHeaders =================-->
<TITLE>Vote.ph : Edit Party Activity</TITLE>
</HEAD>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>EDIT PARTY ACTIVITY</B></DIV>
<BR>
<?PHP if (empty($hassubmit)) { ?>

<FORM ACTION="<?PHP echo $PHP_SELF; ?>" METHOD="post">
<B>Date to Stop Display (YYYY-MM-DD):</B>&nbsp;&nbsp;<INPUT TYPE="text" NAME="date" VALUE="<?PHP echo $activityrow['date']?>" SIZE="12"><BR>
<B>Title:</B>&nbsp;&nbsp;<INPUT TYPE="text" NAME="title" VALUE="<?PHP echo stripslashes(trim($activityrow['title'])); ?>" SIZE="75" MAXLENGTH="120"><BR>
<B>Party:</B>&nbsp;&nbsp;
<SELECT NAME="party_id" SIZE="1">
    <?PHP while ($partyrow = mysql_fetch_array($party)) { ?>
	<OPTION VALUE="<?PHP echo $partyrow['party_id']; ?>" <?PHP if ($activityrow['party_id'] == $partyrow['party_id']) echo "SELECTED" ?>>
...
&nbsp;&nbsp;&nbsp;<INPUT TYPE="radio" NAME="is_national" VALUE="Y" <?PHP if($activityrow['is_national'] == "Y") echo "CHECKED"; ?>>&nbsp;Yes<BR>
&nbsp;&nbsp;&nbsp;<INPUT TYPE="radio" NAME="is_national" VALUE="N" <?PHP if($activityrow['is_national'] == "N") echo "CHECKED"; ?>>&nbsp;No<BR>
<B>NCR Municipality/City:</B>&nbsp;&nbsp;
	<SELECT NAME="ncrmunicity_id" SIZE="1">
		<OPTION VALUE="0">&nbsp;</OPTION>
		<?PHP while ($ncrmunicityrow = mysql_fetch_array($ncrmunicity)) { ?>
			<OPTION VALUE="<?PHP echo $ncrmunicityrow['municity_id']; ?>" <?PHP if ($activityrow['ncrmunicity_id'] == $ncrmunicityrow['municity_id']) echo "SELECTED"; ?>>
...
<B>Province:</B>&nbsp;&nbsp;
	<SELECT NAME="province_id" SIZE="1">
		...
		<?PHP while ($provincerow = mysql_fetch_array($province)) { ?>
			<OPTION VALUE="<?PHP echo $provincerow['province_id']; ?>" <?PHP if ($activityrow['province_id'] == $provincerow['province_id']) echo "SELECTED"; ?>>
...
<B>Provincial Municipality/City:</B>&nbsp;&nbsp;
	<SELECT NAME="municity_id" SIZE="1">
		...
		<?PHP while ($municityrow = mysql_fetch_array($municity)) { ?>
			<OPTION VALUE="<?PHP echo $municityrow['municity_id']; ?>" <?PHP if ($activityrow['municity_id'] == $municityrow['municity_id']) echo "SELECTED"; ?>>
			  <?PHP echo $municityrow['municity']; ?>&nbsp;&nbsp;of&nbsp;&nbsp;<?PHP echo $municityrow['province']; ?>
			</OPTION>
		<?PHP } ?>
	</SELECT><BR>
	<?PHP mysql_free_result($municity); ?>
<B>Source:</B>&nbsp;&nbsp;<INPUT TYPE="text" NAME="source" VALUE="<?PHP echo $activityrow['source']?>" SIZE="50" MAXLENGTH="50"><BR>
<B>Body:</B>&nbsp;&nbsp;<BR>
<TEXTAREA COLS="80" ROWS="25" NAME="body"><?PHP echo stripslashes($activityrow['body']); ?></TEXTAREA><BR>
<BR>
...
<?PHP
	...
	$query = "UPDATE activities SET body = '".addslashes($body).
