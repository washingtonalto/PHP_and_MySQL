...
$query = "SELECT candvicemayors.vicemayor_id, candvicemayors.lastname, candvicemayors.firstname, candvicemayors.middleinitial, candvicemayors.numvotes, candvicemayors.is_proclaimed
          FROM candvicemayors
		  WHERE (candvicemayors.municity_id = ".$municityid.") AND ((candvicemayors.numvotes > 0) OR (candvicemayors.is_proclaimed = 'Y'))
		  ORDER BY candvicemayors.numvotes DESC, candvicemayors.lastname";
$candvicemayors = getqueryresults($query);
 
$query = "SELECT municity.name As municity, vmayresultcomment
          FROM municity
...
<TITLE>Vote.ph : Election Results on Vice Mayoral Candidates of <?PHP echo $municityrow['municity']; ?></TITLE>
</HEAD>
...
<B>Election Results on Vice Mayoral Candidates of <?PHP echo $municityrow['municity']; ?></B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>ELECTION RESULTS ON VICE MAYORAL CANDIDATES OF <?PHP echo strtoupper($municityrow['municity']); ?></B></DIV>
<BR>
<?PHP if (!empty($municityrow['vmayresultcomment'])) { ?>
	<?PHP echo $municityrow['vmayresultcomment']; ?>
<?PHP } ?>
<BR>
