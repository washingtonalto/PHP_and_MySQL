...
$query = "SELECT candcouncilors.councilor_id, candcouncilors.lastname, candcouncilors.firstname, candcouncilors.middleinitial, candcouncilors.numvotes, candcouncilors.counlegdist, candcouncilors.is_proclaimed
          FROM candcouncilors
		  WHERE (candcouncilors.ncrmunicity_id = ".$municityid.") AND ((candcouncilors.numvotes > 0) OR (candcouncilors.is_proclaimed = 'Y'))
		  ORDER BY candcouncilors.numvotes DESC, candcouncilors.lastname";
$candcouncilors = getqueryresults($query);
 
$query = "SELECT nationalcapitalregion.name As municity, counresultcomment
...
<TITLE>Vote.ph : Election Results on Councilor Candidates of <?PHP echo $ncrmunicityrow['municity']; ?></TITLE>
</HEAD>
...
<B>Election Results on Councilor Candidates of <?PHP echo $ncrmunicityrow['municity']; ?></B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>ELECTION RESULTS ON COUNCILOR CANDIDATES OF <?PHP echo strtoupper($ncrmunicityrow['municity']); ?></B></DIV>
<BR>
<?PHP if (!empty($ncrmunicityrow['counresultcomment'])) { ?>
	<?PHP echo $ncrmunicityrow['counresultcomment']; ?>
<?PHP } ?>
<BR>
