...
$query = "SELECT candmayors.mayor_id, candmayors.lastname, candmayors.firstname, candmayors.middleinitial, candmayors.numvotes, candmayors.is_proclaimed
          FROM candmayors
		  WHERE (candmayors.ncrmunicity_id = ".$municityid.") AND ((candmayors.numvotes > 0) OR (candmayors.is_proclaimed = 'Y'))
		  ORDER BY candmayors.numvotes DESC, candmayors.lastname";
$candmayors = getqueryresults($query);
 
$query = "SELECT nationalcapitalregion.name As municity, mayresultcomment
...
<TITLE>Vote.ph : Election Results on Mayoral Candidates of <?PHP echo $ncrmunicityrow['municity']; ?></TITLE>
</HEAD>
...
<B>Election Results on Mayoral Candidates of <?PHP echo $ncrmunicityrow['municity']; ?></B>
</TD>
</TR>
</TABLE>
...
<BR>
<DIV ALIGN="center" STYLE="background-color: #E6E6E6;"><B>ELECTION RESULTS ON MAYORAL CANDIDATES OF <?PHP echo strtoupper($ncrmunicityrow['municity']); ?></B></DIV>
<BR>
<?PHP if (!empty($ncrmunicityrow['mayresultcomment'])) { ?>
	<?PHP echo $ncrmunicityrow['mayresultcomment']; ?>
<?PHP } ?>
<BR>
